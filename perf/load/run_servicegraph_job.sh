...
function service_graph() {
  ...
  pod=$(kubectl get pod --namespace aks-istio-system --selector="app=prometheus" --output jsonpath='{.items[0].metadata.name}')
  kubectl -n aks-istio-system port-forward "$pod" 8060:9090 > /tmp/forward &
